...
class SchemaNormalization {
   ...
   */
  public static String toParsingForm(TypeSchema s) {
    try {
   ...
   * supplied schema.
   */
  public static byte[] parsingFingerprint(String fpName, TypeSchema s) throws NoSuchAlgorithmException {
   ...
   * supplied schema.
   */
  public static long parsingFingerprint64(TypeSchema s) {
    return fingerprint64(toParsingForm(s).getBytes(StandardCharsets.UTF_8));
  }

  private static Appendable build(Map<String, String> env, TypeSchema s, Appendable o) throws IOException {
    boolean firstTime = true;
    SchemaType st = s.getType();
    switch (st) {
